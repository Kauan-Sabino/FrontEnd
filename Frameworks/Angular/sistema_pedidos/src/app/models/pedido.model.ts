...
export class Pedido{
    ...
    calcularTotal(produtos:any[]):number{
        const total = this.itens.reduce((acc,item) =>{ //reduce -> reduz uma lista a um unico item
            ...
            return acc+(prod ? prod.preco*item.quantidade : 0); // se o produto estiver na lista faz ? produto*qtd, o caso não: 0       
        },0); // o acumulado sempre começa em 0
        return total - (total*(this.desconto/100)) // calcula o desconto sobre o valor total encotrado anteriormente
    }
